...
const isDivActive = (army) => {
		return army.divisions.filter(division => division.healt > 0);
}
...
const toArms = (army1, army2) => {
	...
	const fightEvents = (army1, army2, attDivision, vicDivision ) =>{
		...
		console.log([`Conflict -> Division ${attDivision+1} of ${firstArmy.armyName} dealt`,
			`${attacker.damage} damage to divison ${vicDivision +1 } of `,
			`${army2.armyName}`].join(" "))
		console.log([`${Math.floor(attacker.damage/100)} soliders of ${vicDivision +1}`,
			`division of ${army2.armyName} has been killed`,
			`${armySolNum(army2)} still in combat!`].join (" "))
		attacker.lvl += 0.2
